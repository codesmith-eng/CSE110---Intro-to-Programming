...
print("😂🎉🤣👋👋This game was created for fun by Codesmith.")

import random

# Define a list of words to be guessed
words = ["jamaica", "moses", "mosiah", "nigeria", "insect"]
...
while True:
    ...
    # If the guess is incorrect, update the hint
        ...
        for u in range(len(word_list)):
            if guess[u] == word_list[u]:
                hint[u] = guess[u].upper()
            elif guess[u] in word_list:
                hint[u] = guess[u].lower()
            else:
                hint[u] = "_"
        print("Your hint is: " + " ".join(hint))
